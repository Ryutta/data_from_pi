...
if (exists("calc_data") && nrow(calc_data) > 0 && file.exists("campaign_periods.csv")) {
  ...
  if (nrow(campaign_results) > 0) {
    ...
    # 散布図の作成 2 (詰まり本数 vs ジャケット圧力)
    p_campaign_nos_p <- ggplot(campaign_results %>% filter(!is.na(NoS)), aes(x = NoS, y = Max_Jacket_P)) +
      geom_point(aes(color = Type), size = 3, alpha = 0.7) +
      geom_smooth(method = "lm", se = TRUE, color = "darkred", fill = "pink", alpha = 0.3) +
      labs(
        title = "キャンペーン期間ごとの詰まり本数と最大ジャケット圧力の相関",
        subtitle = "各キャンペーンの初期(1ヶ月)・末期(1ヶ月)における最大ジャケット圧力時のデータ",
        x = "詰まり本数 (NoS)",
        y = "最大ジャケット圧力 (M2PI4410W.PV)",
        color = "期間"
      ) +
      theme_minimal()
    ...
    # 散布図の作成 3 (詰まり本数 vs U値)
    p_campaign_nos_u <- ggplot(campaign_results %>% filter(!is.na(NoS)), aes(x = NoS, y = U_Value)) +
      geom_point(aes(color = Type), size = 3, alpha = 0.7) +
      geom_smooth(method = "lm", se = TRUE, color = "darkgreen", fill = "lightgreen", alpha = 0.3) +
      labs(
        title = "キャンペーン期間ごとの詰まり本数とU値の相関",
        subtitle = "各キャンペーンの初期(1ヶ月)・末期(1ヶ月)における最大ジャケット圧力時のデータ",
        x = "詰まり本数 (NoS)",
        y = "総括伝熱係数 (U値)",
        color = "期間"
