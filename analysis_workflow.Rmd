...
定義したタグのリストをループ処理し、WebIDを取得した後、2023年以降の日次データ（`interval = "1d"`）を取得します。
...
if (nrow(all_data) > 0) {
  ...
  if (all(required_tags %in% names(wide_data))) {
    ...
    calc_data <- wide_data %>%
      ...
      select(Timestamp, q, pump_flow, t1, t2, t3_t4, delta_tlm, U) %>%
...
if (exists("calc_data") && nrow(calc_data) > 0) {
  datatable(
    ...
    formatRound(columns = c("q", "pump_flow", "t1", "t2", "t3_t4", "delta_tlm", "U"), digits = 2) %>%
